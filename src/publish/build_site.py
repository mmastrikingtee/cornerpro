from jinja2 import Environment, FileSystemLoader
from pathlib import Path
from datetime import datetime
import sqlite3, urllib.parse, csv
from src.config import DATABASE_URL

def _db():
    db_path = DATABASE_URL.split('///',1)[1] if 'sqlite' in DATABASE_URL else './data/cornerpro.sqlite'
    return sqlite3.connect(db_path)

def slug(s: str):
    return urllib.parse.quote(s.lower().replace(' ', '-'))

def load_predictions():
    pred = {}
    p = Path('data/processed/predictions.csv')
    if not p.exists():
        return pred
    with p.open('r', encoding='utf-8') as f:
        r = csv.DictReader(f)
        for row in r:
            pred[row['bout_id']] = row
    return pred

def build():
    out = Path('site/public'); out.mkdir(parents=True, exist_ok=True)
    env = Environment(loader=FileSystemLoader('site/templates'))

    # Index (the template can list events; we always include timestamp)
    (out/'index.html').write_text(
        env.get_template('index.html').render(timestamp=datetime.utcnow().isoformat()),
        encoding='utf-8'
    )

    con = _db(); cur = con.cursor()
    preds = load_predictions()

    # Build a page for EVERY future event
    today = datetime.utcnow().date().isoformat()
    events = cur.execute(
        'SELECT event_id, org, event_date, name FROM events WHERE event_date >= ? ORDER BY event_date',
        [today]
    ).fetchall()

    for event_id, org, dt, name in events:
        rows = cur.execute(
            'SELECT b.bout_id, fa.name, fb.name, b.weight_class '
            'FROM bouts b '
            'JOIN fighters fa ON fa.fighter_id=b.fighter_a_id '
            'JOIN fighters fb ON fb.fighter_id=b.fighter_b_id '
            'WHERE b.event_id=?',
            [event_id]
        ).fetchall()

        out_rows = []
        for (bout_id, a_name, b_name, weight) in rows:
            pr = preds.get(bout_id)
            if pr:
                p_a = float(pr['p_a']); p_b = float(pr['p_b'])
                odds_a = int(pr['odds_a']); odds_b = int(pr['odds_b'])
            else:
                p_a = p_b = 0.5; odds_a = odds_b = -100
            out_rows.append({
                'bout_id': bout_id,
                'a_name': a_name, 'b_name': b_name,
                'weight_class': (weight or '').strip(),
                'p_a': p_a, 'p_b': p_b,
                'odds_a': odds_a, 'odds_b': odds_b
            })

        # Even if a card has zero parsed bouts, still publish a page (empty table)
        card_html = env.get_template('card.html').render(
            event_name=name, event_date=dt, rows=out_rows
        )
        card_dir = out / 'cards' / f"{dt}-{org.lower()}-{slug(name)}"
        card_dir.mkdir(parents=True, exist_ok=True)
        (card_dir/'index.html').write_text(card_html, encoding='utf-8')

    con.close()
    print(f"Built site to {out}/ with {len(events)} event pages.")

if __name__ == '__main__':
    build()
